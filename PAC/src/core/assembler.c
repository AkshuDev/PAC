...
static size_t default_section_alignment(const char* name) {
    if (strcmp(name, ".text") == 0) return 16;
    if (strcmp(name, ".data") == 0) return 8;
    if (strcmp(name, ".rodata") == 0) return 8;
    if (strcmp(name, ".bss") == 0) return 8;
    if (strcmp(name, ".stack") == 0) return 16;
    ...
    for (size_t i = 0; i < root->child_count; i++) {
        ...
        switch (node->type) {
						...
						switch (littype) {
							...
							case LIT_STRING: {
								...
								if (val_size + len >= val_max_size) {
									while (val_size + len >= val_max_size) val_max_size *= 2;
			...
			case AST_RESERVE: {
				...
				size_t size = token_type_size(node->reserve.type);
				if (size == 0) {
